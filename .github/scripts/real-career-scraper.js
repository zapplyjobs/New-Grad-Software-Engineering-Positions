...
          job_posted_at: convertDateToRelative(job.postingDate),
          job_employment_type: "FULLTIME",
        }));
    },
  },
...
          job_posted_at: convertDateToRelative(safeISOString(job.t_create * 1000)),
          job_employment_type: "FULLTIME",
        }));
    },
  },
...
async function fetchSimplifyJobsData() {
  try {
    ...
    const activeJobs = data
      ...
      .map((job) => {
        // Console log the raw data first
       
        // Calculate the relative date
        const relativeDate = convertDateToRelative(safeISOString(job.date_posted * 1000));
...
levelFilteredJobs.forEach((job, index) => {
  console.log(`${index + 1}. Name: ${job.job_title|| job.title || 'N/A'} | Date Posted: ${job.job_posted_at || job.posted || job.date || 'N/A'}`);
});  
